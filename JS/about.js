...
window.addEventListener('DOMContentLoaded', () => {
  document.querySelector('.text').innerHTML = `.title {opacity: 1;}`;
  setTimeout(() => {document.querySelector('.text').innerHTML += `.subtitle {opacity: 1;}`; setTimeout(() => {document.querySelector('.text').innerHTML += `.main-text {opacity: 1;}`}, 600);}, 1100);
});
